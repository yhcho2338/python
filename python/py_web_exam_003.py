...
class main_news(basic_news):
    ...
    def __str__(self):
        result_str = []
        result_str.append('이미지주소: ' + self.img_url)
        result_str.append('관련링크 목록: ' + str(self.real_links))
...
def get_html_from_daum_news(news_categories):   
    surl = news_baseURL + news_categories
    req = urllib3.PoolManager()
    return req.request('GET',  surl).data     


# get_common_news 두번째 선언
def get_common_news(news_obj):
    img_url = news_obj.select('.thumb_g')[0].get('src').split('//')[1]
...
def arg_main():
    ...
    parser.add_argument('--categories', nargs='+', help='카테고리 목록에 공백을 두고 나열', required=True)
    args = parser.parse_args()
    categories = args.categories

    for key in categories:
        result = get_html_from_daum_news(news_categories[key])
        parsed = BeautifulSoup(result, 'html.parser')
